...
Plans:
- [ ] 01-01: Initialize project structure — `app/`, `static/`, `templates/`, `Dockerfile`, `docker-compose.yml`, `requirements.txt`, `.env.example`, `.gitignore`
...
Plans:
- [ ] 03-01: Implement license router — GET/POST for create, GET/PUT for edit (HTMX inline row swap), DELETE with confirmation, row and edit-row fragment templates
...
Plans:
- [ ] 04-01: Implement settings service and router — `AppSettings` key-value table, DB-over-env precedence chain, settings page with HTMX form save, `GET /settings` and `POST /settings`
